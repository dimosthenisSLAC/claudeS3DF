...
echo "=== OCEAN 3.2.0.1: IrO2 Ir L-edge — 240 cores (2 nodes) scaling test ==="
echo "Date: $(date)"
...
WORKDIR="/sdf/scratch/users/d/dsokaras/ocean321_240c_${SLURM_JOB_ID}"
mkdir -p "$WORKDIR"
cd "$WORKDIR"
...
# Multi-node: add tcp transport alongside vader for inter-node communication
sed -i "s|para_prefix{.*}|para_prefix{ mpirun -n $SLURM_NTASKS --mca btl self,vader,tcp --mca pml ob1 }|" IrO2.in
...
echo "=========================================="
echo "  Running OCEAN 3.2.0.1 — 240 cores / 2 nodes"
echo "=========================================="
T0=$(date +%s)
...
# Copy results
RESULT_DIR="$HOME/claudeS3DF/results/ocean_iro2_scaling/240cores"
mkdir -p "$RESULT_DIR"
